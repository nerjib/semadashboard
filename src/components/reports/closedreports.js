...
const ClosedReports=()=>{
    ...
    // const [tableData, setReport] = useState([]),
     ...
     useEffect(()=>{
         axios.get('https://kd-sema.herokuapp.com/api/v1/reports/getclosedreports')
         .then(res=>{
     ...
     if(reports.length>0){
         Object.keys(reports).map((e,i)=>{
            ...
            reports[e].type,
            <a href={`#draft/followup/closed/${reports[e].id}`}><button className='btn btn-primary'>View</button></a>],)
         })
    ...
    return (
        <div>
            <GridContainer>
                ...
                tableHead={["SN", "Event", "Location", "Date", "Time","Activity life  time(Days)", "Source",""]}
