    ...
    // const [tableData, setReport] = useState([]),
     ...
     if(reports.length>0){
         Object.keys(reports).map((e,i)=>{
            ...
            Math.round((((new Date(today)).getTime()) - (new Date(reports[e].date)).getTime())/day1).toFixed(0),
            reports[e].type,
            reports[e].category,
            <a href={`#draft/followup/lga/${reports[e].id}`}><button className='btn btn-primary'>View</button></a>],)
         })
                 ...
                 <GridItem xs={12} sm={10} md={12}>
                     <Card>
                        <CardHeader color="warning" stats icon>
                                     <h2 className={classes.cardTitle}>Closed Reports</h2>
                        </CardHeader>
                    </Card>
                    <CardBody>
                    <Table
                tableHeaderColor="warning"
                tableHead={["SN", "Event", "Location", "Date", "Time","Activity life  time(Days)", "Source","Stage",""]}
